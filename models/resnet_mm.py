...
class BasicBlock_mm(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(BasicBlock_mm, self).__init__()
...
class ResNet_mm(nn.Module):
    def __init__(self, block, num_blocks, mm = True, num_classes=10):
        ...
        self.linear_mm = nn.Linear(num_classes, num_classes)
    ...
    def forward(self, x):
        ...
        out = self.linear(out)
        out = self.linear_mm(out)
        return out

def ResNet8_mm():
    return ResNet_mm(BasicBlock_mm, [1,1,1,1])

def ResNet34_mm():
    return ResNet_mm(BasicBlock_mm, [3,4,6,3])